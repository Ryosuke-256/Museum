...
document.addEventListener('mousemove', (e)=> {
    if (controls.isLocked) {
        ...
        const currentRotationX = controls.getObject().rotation.x;
        // 回転角度が制限を超えている場合は制限内に収める
        if (currentRotationY < minRotationY) {
            controls.getObject().rotation.y = minRotationY;
        } else if (currentRotationY > maxRotationY) {
            controls.getObject().rotation.y = maxRotationY;
        }
        if (currentRotationX < minRotationX){
            controls.getObject().rotation.x = minRotationX
        } else if (currentRotationX > maxRotationX) {
            controls.getObject().rotation.x = maxRotationX
        }
    }
})
